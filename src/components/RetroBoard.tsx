...
import { MessageSquare, Users as UsersIcon, RefreshCw, Crown } from 'lucide-react';
...
export const RetroBoard: React.FC = () => {
  ...
  const { room, loading: roomLoading, updateRoom, setCreator } = useRoom(roomId || '');
  const { cards, loading: cardsLoading, addCard, updateCard, voteCard } = useRetroCards(
    roomId || '', 
    room?.settings.sortBy || 'timestamp',
    room?.settings.sortOrder || 'desc'
  );
  const participants = useParticipants(roomId || '', userName, userId, isCreator);
  ...
  useEffect(() => {
    ...
    const storedCreatorId = localStorage.getItem(`retroboard_creator_${roomId}`);
  ...
  const handleAddCard = (text: string, category: 'start' | 'stop' | 'continue', customFields?: { [key: string]: string }) => {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Room: <span className="font-semibold text-blue-600">{roomId}</span></p>
                    {isCreator && (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg border-2 border-blue-100 p-4">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <p><span className="font-medium">Total Cards:</span> {cards.length}</p>
                <p><span className="font-medium">Your Name:</span> {userName}</p>
                <p><span className="font-medium">Room ID:</span> {roomId}</p>
                <p><span className="font-medium">Sort:</span> {room.settings.sortBy} ({room.settings.sortOrder})</p>
              </div>
            </div>
          </div>
        </div>
